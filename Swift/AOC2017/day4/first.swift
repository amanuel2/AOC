...
func phrase(_ password: String) -> Int 
{
    let passphrase = password.split(separator: "\n")
    var res = 0

    outer: for phrase in passphrase {
        let strs = phrase.split(separator: " ")
        var arr: Set<String> = Set()

        for str in strs {
            if arr.contains(String(str)) {continue outer}
            arr.update(with: String(str))
        }
