...
func solve(_ arr: [Int:Int], delay: Int) -> Int
{
    // column : range_pos
    var pos: [Int: (Int, DIR)] = [:]
    ...
    for _ in 0...delay { advance(arr, &pos) }
...
while true {
    var caught = false
    for (depth, range) in mp {
        // forward and back
        // So, the total cycle for a scanner to return to its initial position is 2 * (range - 1).
